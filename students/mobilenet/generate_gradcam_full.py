...
KD_CKPT       = _HERE / "checkpoints" / "mobilenet_kd.pth"
BASELINE_CKPT = _HERE / "checkpoints" / "mobilenet_baseline.pth"
...
def save_figure(img, raw_maps, pred_names, true_class, stem):
    ...
    for ax, (key, label) in zip(axes[1:], [
        ...
        ("kd_student", "KD Student (MobileNetV2)"),
...
def main():
    ...
    kd_student = load_mobilenet(KD_CKPT)
    baseline   = load_mobilenet(BASELINE_CKPT)

    cam_teacher  = GradCAM(model=teacher,    target_layers=[teacher.layer4[-1]])
    cam_kd       = GradCAM(model=kd_student, target_layers=[kd_student.features[-1]])
    cam_baseline = GradCAM(model=baseline,   target_layers=[baseline.features[-1]])
    model_cams   = [
