"""
Baseline training: MobileNetV2 on ImageNette with hard labels only.

Loss = CrossEntropy(student_logits, hard_labels) — no teacher, no soft labels.

Identical dataset, preprocessing, optimizer, scheduler, and number of epochs
as train_kd.py so the two results are directly comparable.
...
def main():
    ...
    csv_path  = results_dir / "logs" / "mobilenet_baseline_training_log.csv"
    ckpt_path = ckpt_dir / "mobilenet_baseline.pth"
